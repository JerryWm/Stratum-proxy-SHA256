...
class StratumConfig {
	...
	parseConfig(pool_info) {
		...
		if ( typeof this.wallet_address !== "string" || !this.wallet_address.length ) {
			this.logger.error(this.error_text = "Invalid wallet address");
			return false;
		}
	...
	parseConfigPoolAddress(_pool_address) {
		...
		if ( typeof pool_address !== "string" || !pool_address.length ) {
			this.logger.error(this.error_text = "Invalid pool address");
			return false;
		}
		...
		if ( !tmphp ) {
			this.logger.error(this.error_text = "Invalid pool address \"" + _pool_address + "\"");
			return false;
		}
