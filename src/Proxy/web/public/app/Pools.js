define(["app/Common", "app/CommonView", "app/UnPackArray"], function(Common, CommonView, UnPackArray) {
	...
	PoolList.prototype.syncPool = function(id) {
		...
		this.dataTable.row(poolWrap.dtRowIndex).data([
			...
			Common.doubleOrNA(pool.difficulty, 3),
			Common.doubleOrNA_kStyle(pool.difficulty_real),
			
			Common.intOrNA(pool.worker_count),
			Common.intOrNA(pool.job_count),
			CommonView.viewAcceptedRejectedShares(pool.accepted_share_count, pool.rejected_share_count),
			Common.doubleOrNA_kStyle(pool.hash_count),
			...
			<div class="popover" role="tooltip" style="max-width: none;">
				...
				<div style="padding: 10px">
					<table class="table table-hover">
						...
						<tr><td>Jobs</td><td>${Common.intOrNA(pool.job_count)}</td></tr>
						...
						<tr><td>Hashes</td><td>${Common.doubleOrNA_kStyle(pool.hash_count)}</td></tr>
